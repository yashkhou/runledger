...
export function verifyRunAttestation(attestation: SignedRunAttestation, expected?: { run?: FlightRun; policy?: RunPolicy; publicKey?: string }) {
  ...
  const signatureOk = verify(null, Buffer.from(canonicalize(attestation.payload)), createPublicKey(publicKeyPem), Buffer.from(attestation.signature, "base64"));
  if (!signatureOk) return { ok: false, reason: "bad-signature" };
  if (expected?.run) {
    if (sha256Canonical(expected.run) !== attestation.payload.runDigest) return { ok: false, reason: "run-digest-mismatch" };
    if ((expected.run.events.at(-1)?.hash ?? "GENESIS") !== attestation.payload.finalEventHash) return { ok: false, reason: "run-head-mismatch" };
  }
  if (expected?.policy) {
    if (sha256Canonical(expected.policy) !== attestation.payload.policyDigest) return { ok: false, reason: "policy-digest-mismatch" };
    if (expected.run) {
